...
def register_patient(payload: PatientCreate, db: Session = Depends(get_db), _=Depends(get_current_active_user)):
...
def update_patient(
    patient_id: str, payload: PatientUpdate, db: Session = Depends(get_db), _=Depends(get_current_active_user)
):
...
def delete_patient(patient_id: str, db: Session = Depends(get_db), _=Depends(get_current_active_user)):
    patient = get_or_404(db, Patient, patient_id, "Patient")
    db.delete(patient)
    db.commit()
...
def delete_emergency_contact(
    patient_id: str, contact_id: str, db: Session = Depends(get_db), _=Depends(get_current_active_user)
):
