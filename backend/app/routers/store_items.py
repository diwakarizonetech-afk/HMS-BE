...
def create_item(payload: ItemMasterCreate, db: Session = Depends(get_db), _=Depends(get_current_active_user), _perm=_perm_create):
...
def create_vendor(payload: VendorCreate, db: Session = Depends(get_db), _=Depends(get_current_active_user), _perm=_perm_create):
    data = payload.model_dump()
    vendor = Vendor(**data)
    db.add(vendor)
    db.commit()
