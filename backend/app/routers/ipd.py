...
def create_bed(payload: BedCreate, db: Session = Depends(get_db), _=Depends(get_current_active_user)):
...
def update_bed(bed_id: str, payload: BedUpdate, db: Session = Depends(get_db), _=Depends(get_current_active_user)):
...
def allocate_bed(
    bed_id: str, payload: BedAllocateRequest, db: Session = Depends(get_db), _=Depends(get_current_active_user)
):
    bed = get_or_404(db, Bed, bed_id, "Bed")
...
def release_bed(bed_id: str, db: Session = Depends(get_db), _=Depends(get_current_active_user)):
    bed = get_or_404(db, Bed, bed_id, "Bed")
    bed.status = "Cleaning"
...
def delete_bed(bed_id: str, db: Session = Depends(get_db), _=Depends(get_current_active_user)):
    bed = get_or_404(db, Bed, bed_id, "Bed")
    db.delete(bed)
    db.commit()
...
def admit_patient(
    payload: IPDAdmissionCreate, db: Session = Depends(get_db), _=Depends(get_current_active_user)
):
...
def discharge_patient(admission_id: str, db: Session = Depends(get_db), _=Depends(get_current_active_user)):
    admission = get_or_404(db, IPDAdmission, admission_id, "IPD admission")
