...
def list_stock_inward(db: Session = Depends(get_db), _=Depends(get_current_active_user)):
    return db.scalars(select(StockInward).order_by(StockInward.created_at.desc())).all()


@router.post("/stock-inward", response_model=StockInwardOut, status_code=status.HTTP_201_CREATED)
def create_stock_inward(
    payload: StockInwardCreate, db: Session = Depends(get_db), _=Depends(get_current_active_user), _perm=_perm_create
):
...
def list_stock_outward(db: Session = Depends(get_db), _=Depends(get_current_active_user)):
    return db.scalars(select(StockOutward).order_by(StockOutward.created_at.desc())).all()


@router.post("/stock-outward", response_model=StockOutwardOut, status_code=status.HTTP_201_CREATED)
def create_stock_outward(
    payload: StockOutwardCreate, db: Session = Depends(get_db), _=Depends(get_current_active_user), _perm=_perm_create
):
    data = payload.model_dump()
    data["date"] = data.get("date") or today_str()
...
def list_stock_transfer(db: Session = Depends(get_db), _=Depends(get_current_active_user)):
    return db.scalars(select(StockTransfer).order_by(StockTransfer.created_at.desc())).all()


@router.post("/stock-transfer", response_model=StockTransferOut, status_code=status.HTTP_201_CREATED)
def create_stock_transfer(
    payload: StockTransferCreate, db: Session = Depends(get_db), _=Depends(get_current_active_user), _perm=_perm_create
):
    data = payload.model_dump()
...
def list_stock_adjustment(db: Session = Depends(get_db), _=Depends(get_current_active_user)):
    return db.scalars(select(StockAdjustment).order_by(StockAdjustment.created_at.desc())).all()


@router.post("/stock-adjustment", response_model=StockAdjustmentOut, status_code=status.HTTP_201_CREATED)
def create_stock_adjustment(
    payload: StockAdjustmentCreate, db: Session = Depends(get_db), _=Depends(get_current_active_user), _perm=_perm_create
):
    data = payload.model_dump()
