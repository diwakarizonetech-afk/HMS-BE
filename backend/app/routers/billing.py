...
def _razorpay_client() -> Any:
    if razorpay is None:
        raise HTTPException(
            ...
            detail="Razorpay Python library is not installed in this environment. Run 'pip install razorpay' or use Mock payment confirmation.",
        )
...
def _razorpay_qrcode_resource(client: Any = None) -> Any:
    rp_client = client or _razorpay_client()
    qr_res = getattr(rp_client, "qrcode", None) or getattr(rp_client, "qr_code", None)
    if qr_res is not None:
        return qr_res
    if razorpay is not None and hasattr(razorpay, "Qrcode"):
        return razorpay.Qrcode(rp_client)
    from razorpay.resources.qrcode import Qrcode  # type: ignore
    return Qrcode(rp_client)
...
def create_razorpay_qr_payment(payload: PaymentQrCreateSchema, db: Session = Depends(get_db)):
    ...
    if settings.RAZORPAY_KEY_ID and settings.RAZORPAY_KEY_SECRET and razorpay is not None:
        request_payload = {
            "type": "upi_qr",
        ...
        try:
            data = _razorpay_qrcode_resource().create(request_payload)
...
def sync_razorpay_payment_transaction(transaction_id: str, db: Session = Depends(get_db)):
    ...
    try:
        data = _razorpay_qrcode_resource().fetch_all_payments(tx.provider_reference, {"count": 10})
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
    ...
    captured = next(
        (
            ...
            if item.get("status") == "captured" and int(item.get("amount") or 0) >= round(tx.amount * 100)
        ),
        None,
    )
    if not captured:
