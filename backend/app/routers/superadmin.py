...
def normalize_role_value(role_str: str | None) -> str:
    if not role_str:
        return "reception"
    clean = role_str.strip().lower().replace("userrole.", "").replace("-", "_").replace(" ", "_")
    if "billing" in clean or "finance" in clean or "cashier" in clean or "accounts" in clean:
        return "billing"
    if "pharmacy" in clean or "pharmacist" in clean:
        return "pharmacy"
    if "reception" in clean or "receptionist" in clean:
        return "reception"
    if "lab" in clean:
        return "lab"
    if "store" in clean:
