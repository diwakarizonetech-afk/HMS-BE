...
def get_branches(db: Session = Depends(get_db), _=_any_auth):
...
def normalize_role_value(role_str: str | None) -> str:
    if not role_str:
        return "doctor"
    clean = str(role_str).strip().lower().replace(" ", "_")
    mapping = {
        "doctor": "doctor",
        "nurse": "nurse",
        "reception": "reception",
    ...
    return mapping.get(clean, "doctor")
...
def update_user(user_id: str, payload: UserUpdate, db: Session = Depends(get_db), _=_admin_only):
    ...
    db.commit()
    db.refresh(user)
    log_audit(f"PUT /users/{user_id}", payload, payload.model_dump(exclude_unset=True), user, user)
    notify_user_or_role(
