...
def list_doctors(
        ...
        for du in doc_users:
            ...
            if not doc:
                new_doc = Doctor(
                    ...
                    specialization="General Physician" if "General" in (du.department or "") else "Specialist Consultant",
                    room_no="OPD-101",
                ...
                if not doc.available_days:
                    doc.available_days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]
                if not doc.slots or len(doc.slots) < 8:
                    doc.slots = [
                        "08:00 AM", "08:30 AM", "09:00 AM", "09:30 AM",
                        "10:00 AM", "10:30 AM", "11:00 AM", "11:30 AM",
                        "12:00 PM", "12:30 PM", "02:00 PM", "02:30 PM",
    ...
    # 1. Strict filter: matching department and branch
    strict_stmt = select(Doctor)
    if department and department != "All":
        strict_stmt = strict_stmt.where(func.lower(Doctor.department) == department.lower())
    if target_branch and target_branch.lower() != "all":
        strict_stmt = strict_stmt.where(
            (func.lower(Doctor.branch) == target_branch.lower()) |
            (func.lower(Doctor.branch) == "main branch") |
            (func.lower(Doctor.branch).contains("main")) |
            (Doctor.branch.is_(None)) |
            (Doctor.branch == "")
        )
    docs = list(db.scalars(strict_stmt.offset(skip).limit(limit)).all())

    # 2. Fallback 1: Match department across all branches if branch-filtered query returns empty
    if not docs and department and department != "All":
        dept_stmt = select(Doctor).where(func.lower(Doctor.department) == department.lower()).offset(skip).limit(limit)
        docs = list(db.scalars(dept_stmt).all())

    # 3. Fallback 2: Match branch across all departments if department-filtered query returns empty
    if not docs and target_branch and target_branch.lower() != "all":
        branch_stmt = select(Doctor).where(
            (func.lower(Doctor.branch) == target_branch.lower()) |
            (func.lower(Doctor.branch) == "main branch") |
            (func.lower(Doctor.branch).contains("main")) |
            (Doctor.branch.is_(None)) |
        ...
        docs = list(db.scalars(branch_stmt).all())

    # 4. Fallback 3: Return all available doctors if no match found
    if not docs:
        docs = list(db.scalars(select(Doctor).offset(skip).limit(limit)).all())
