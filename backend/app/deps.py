...
def get_own_lab_department(
    ...
    role, or for a lab user with no section assigned yet — same "None = don't
    scope" contract as get_own_doctor_id() / get_own_nurse_ward().

    See CHANGELOG.md Phase 13/14 for the definition this relies on: for a lab
    tech, "department" means their test-catalog section (matches
    LabTestMaster.department values like Pathology/Microbiology/
    Biochemistry — a real central-lab section structure), NOT the *ordering
    clinical department* that happens to share the same column name on
    SampleCollection.department / LabReport.department (that field records
    which clinical department ordered the test, e.g. "General Medicine" or
    "Cardiology", and is unrelated to which lab section processes it).
    """
    if current_user.role != UserRole.lab:
        return None
    return current_user.department if current_user.department and current_user.department != "Laboratory" else None
