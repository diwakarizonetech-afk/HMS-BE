...
import { Search, UserCheck, X, Building2, Loader2 } from 'lucide-react';
...
export const PatientSearch: React.FC<PatientSearchProps> = ({
  ...
  // Trigger server-side lookup on search query
  useEffect(() => {
  ...
  const matchingPatients = patients.filter((p) => {
    ...
    // Matches for Token / Apt ID / IP No
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">
            ...
            {activeBranch && activeBranch !== 'All' && (
              <span className="font-semibold text-blue-600 ml-1">[{activeBranch}]</span>
            )}
          </p>
        </div>
      ...
      <div className="relative">
        <div className="relative">
          ...
          <input
            ...
            placeholder={
              activeBranch && activeBranch !== 'All'
                ? `Search patients in ${activeBranch} by UHID, Name, Token...`
                : 'Search by UHID, Patient Name, Token, or Appointment ID...'
            }
            value={searchTerm}
            onChange={(e) => {
          ...
          <div className="absolute top-full left-0 right-0 z-30 mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl max-h-60 overflow-y-auto divide-y divide-slate-100">
            {matchingPatients.length > 0 ? (
              matchingPatients.map((p) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {(p.isEmergency || (p.status || '').toLowerCase() === 'emergency' || (p.category || '').toLowerCase() === 'emergency') && (
                      <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-rose-100 text-rose-700 border border-rose-200 uppercase animate-pulse">
                        EMERGENCY
                      </span>
                    )}
                    <span
              ...
              <div className="p-4 text-center text-xs text-slate-400">
                No matching patient found{activeBranch && activeBranch !== 'All' ? ` in ${activeBranch}` : ' in database'}.
              </div>
            )}
          </div>
        )}
