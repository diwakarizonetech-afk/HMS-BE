...
export async function fetchPatientsApi(): Promise<Patient[]> {
  const list = await apiRequest<any[]>('/patients');
  return list.map((p) => ({
    id: p.id,
    uhid: p.uhid,
...
export async function fetchDoctorsApi(): Promise<Doctor[]> {
  const list = await apiRequest<any[]>('/doctors');
  return list.map((d) => ({
    id: d.id,
    name: d.name,
...
export async function createDoctorApi(doc: any): Promise<Doctor> {
  const payload = {
    ...
    slots: doc.slots || ['09:00 AM', '10:00 AM', '11:00 AM', '02:00 PM', '03:00 PM'],
...
export async function fetchAppointmentsApi(): Promise<Appointment[]> {
  const list = await apiRequest<any[]>('/appointments');
  return list.map((a) => ({
    id: a.id,
...
export async function createAppointmentApi(apt: Omit<Appointment, 'id' | 'status' | 'createdDate'>): Promise<Appointment> {
  const payload = {
...
// --- Queue & Walk-In API ---
export async function fetchQueueApi(): Promise<QueueItem[]> {
  const list = await apiRequest<any[]>('/queue');
  return list.map((q) => ({
    id: q.id,
    ...
    status: q.status,
    waitingTimeMinutes: q.waiting_time_minutes,
    timeIssued: q.time_issued,
  }));
}

export async function registerWalkInApi(patientUhid: string, patientName: string, department: string, doctorName: string): Promise<WalkInToken> {
  const payload = {
...
// --- IPD Beds API ---
export async function fetchBedsApi(): Promise<Bed[]> {
...
export async function fetchIpdAdmissionsApi(): Promise<IPDAdmission[]> {
  const list = await apiRequest<any[]>('/ipd-admissions');
  return list.map((a) => ({
    id: a.id,
    patientUhid: a.patient_uhid,
    patientName: a.patient_name,
...
// --- Store & Inventory API ---
export async function fetchStoreItemsApi(): Promise<any[]> {
  return apiRequest<any[]>('/store/items');
}
...
export async function fetchPurchaseOrdersApi(): Promise<any[]> {
  return apiRequest<any[]>('/store/purchase-orders');
}
...
export async function fetchVendorsApi(): Promise<any[]> {
  return apiRequest<any[]>('/store/vendors');
}
...
export async function fetchGoodsReceiptsApi(): Promise<any[]> {
  return apiRequest<any[]>('/store/goods-receipts');
}
...
export async function fetchStockInwardApi(): Promise<any[]> {
  return apiRequest<any[]>('/store/stock-inward');
}
...
export async function fetchStockOutwardApi(): Promise<any[]> {
  return apiRequest<any[]>('/store/stock-outward');
}
...
export async function fetchStockTransferApi(): Promise<any[]> {
  return apiRequest<any[]>('/store/stock-transfer');
}
...
export async function fetchStockAdjustmentApi(): Promise<any[]> {
  return apiRequest<any[]>('/store/stock-adjustment');
}
...
export async function fetchInvoicesApi(): Promise<any[]> {
  return apiRequest<any[]>('/pharmacy/invoices');
}
