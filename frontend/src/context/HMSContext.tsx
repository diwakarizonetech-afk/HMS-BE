...
interface HMSContextType {
  ...
  bookAppointment: (apt: Omit<Appointment, 'id' | 'status' | 'createdDate'>) => Promise<Appointment>;
...
export const HMSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadBackendData = async () => {
    ...
    // ── Store / Pharmacy ──────────────────────────────────────────────────────
    if (role === 'store' || role === 'store_manager' || role === 'store manager' || role === 'pharmacy') {
      try {
    ...
    if (role === 'nurse') {
      try {
        const [pts, bds, adm, depts, docs, notifs] = await Promise.all([
          fetchPatientsApi().catch(() => null),
          fetchBedsApi().catch(() => null),
          fetchIpdAdmissionsApi().catch(() => null),
          fetchDepartmentsApi().catch(() => null),
    ...
    try {
      const [pts, depts, docs, apts, qItems, bds, adm, notifs] = await Promise.all([
        fetchPatientsApi().catch(() => null),
        fetchDepartmentsApi().catch(() => null),
        fetchDoctorsApi().catch(() => null),
        fetchAppointmentsApi().catch(() => null),
        fetchQueueApi().catch(() => null),
        fetchBedsApi().catch(() => null),
        fetchIpdAdmissionsApi().catch(() => null),
        fetchNotificationsApi().catch(() => null),
      ]);
  ...
  // Appointments
  const bookAppointment = async (aptData: Omit<Appointment, 'id' | 'status' | 'createdDate'>): Promise<Appointment> => {
    try {
      const created = await createAppointmentApi(aptData);
      ...
      addToast('success', 'Appointment Booked', `Booked with ${created.doctorName} on ${created.date}`);
      return created;
    } catch (err) {
  ...
  const registerWalkIn = async (patientUhid: string, patientName: string, department: string, doctorName: string): Promise<WalkInToken> => {
    try {
      const token = await registerWalkInApi(patientUhid, patientName, department, doctorName);
  ...
  const releaseBed = async (bedId: string) => {
    try {
      ...
      setBeds((prev) => prev.map((b) => (b.id === bedId ? mapBedResponse(updatedBed) : b)));
      addToast('info', 'Bed Released', 'Bed set to cleaning status');
    } catch (err) {
      console.error('releaseBed failed:', err);
      addToast('error', 'Release Failed', 'Could not save the bed release to the server. Please try again.');
      throw err;
    }
  };
