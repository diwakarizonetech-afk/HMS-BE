...
const API_BASE = `${import.meta.env.VITE_API_URL}/api/v1/pharmacy`;
  ...
  const refreshData = async () => {
    try {
      ...
      const [catRes, medRes, batRes, purRes, rxRes, invRes, crRes, srRes] = await Promise.all([
        ...
        fetch(`${API_BASE}/prescription-orders`, { headers }).then(r => r.ok ? r.json() : []),
        fetch(`${API_BASE}/pos-invoices`, { headers }).then(r => r.ok ? r.json() : []),
