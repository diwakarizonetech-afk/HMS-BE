...
export const SuperAdminProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadSuperAdminData = async () => {
    ...
    // Unconditionally fetch leaves & shift rotations from backend DB for all logged-in portal roles
    try {
      const [lvs, sfts] = await Promise.all([
        fetchLeavesApi().catch(() => []),
        fetchShiftsApi().catch(() => []),
      ]);

      if (lvs && Array.isArray(lvs) && lvs.length > 0) {
        const fetched: LeaveRequest[] = lvs.map((l: any) => ({
          id: l.id,
      ...
      if (sfts && Array.isArray(sfts) && sfts.length > 0) {
        const fetchedShifts: ShiftRotation[] = sfts.map((s: any) => ({
          id: s.id,
        ...
        setShiftRotations((prev) => {
          const existingIds = new Set(fetchedShifts.map((f) => f.id));
          const extra = prev.filter((p) => !existingIds.has(p.id));
          return [...fetchedShifts, ...extra];
        });
      }
    } catch (err) {
      console.warn('Leaves/Shifts sync warning:', err);
    }
  ...
  const updateLeaveStatus = async (id: string, approvalStatus: LeaveRequest['approvalStatus']) => {
    ...
    try {
      ...
      console.warn('Backend leave status sync warning:', err);
    }
  };
  ...
  const deleteLeaveRequest = async (id: string) => {
    ...
    try {
      ...
      console.warn('Backend delete leave sync warning:', err);
    }
  };
