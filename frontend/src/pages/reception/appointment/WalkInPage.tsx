...
export const WalkInPage: React.FC = () => {
  ...
  React.useEffect(() => {
    const available = doctors.filter((d) => d.department === selectedDept);
    if (available.length > 0) {
      if (!available.some((d) => d.name === selectedDoctorName)) {
        setSelectedDoctorName(available[0].name);
      }
    } else if (doctors.length > 0 && !selectedDoctorName) {
