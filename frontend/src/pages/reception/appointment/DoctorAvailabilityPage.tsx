...
export const DoctorAvailabilityPage: React.FC = () => {
  const { doctors, departments, branches } = useHMS();
  const { user } = useAuth();
  ...
  const filteredDoctors = React.useMemo(() => {
    ...
    const selDeptClean = (deptFilter || '').replace(/\s*\([^)]*\)/g, '').toLowerCase().trim();
    ...
    let matches = doctors.filter((doc) => {
      ...
      const matchesDept = deptFilter === 'All' || docDept.includes(selDeptClean) || selDeptClean.includes(docDept);
    ...
    // 2. Fallback: Show branch doctors or selected doctor if strict match returns empty
    return doctors.filter((doc) => selectedDoctorId === 'All' || doc.id === selectedDoctorId);
  }, [doctors, branchFilter, deptFilter, selectedDoctorId]);
