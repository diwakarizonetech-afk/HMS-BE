...
export const NursingNotesPage: React.FC = () => {
  const { notes, vitals, medications, addNursingNote, deleteNursingNote, getPatientNotesTimeline, selectedBranch } = useNurse();
  const { patients, appointments, ipdAdmissions, beds, addToast } = useHMS();
  const { user } = useAuth();
  ...
  const handleSaveNotes = (e: React.FormEvent) => {
    ...
    addNursingNote({
      ...
      recordedBy: user?.name ? `Nurse (${user.name})` : 'Nurse Staff',
  ...
  // Filtered Notes Table with Priority Sorting
  const filteredNotes = useMemo(() => {
    const matched = notes.filter((n) => {
      ...
      const matchesBranch = matchBranch(n.branch, activeBranch);
    ...
    return matched.sort((a, b) => {
      ...
      const isAEmergency = Boolean(patA?.isEmergency || (patA?.status || '').toLowerCase() === 'emergency' || a.patientCondition === 'Critical');
      const isBEmergency = Boolean(patB?.isEmergency || (patB?.status || '').toLowerCase() === 'emergency' || b.patientCondition === 'Critical');
      if (isAEmergency !== isBEmergency) {
        return isAEmergency ? -1 : 1; // emergency first
      }
      return 0;
    });
  }, [notes, tableSearch, activeBranch, patients]);
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {paginatedNotes.length > 0 ? (
                paginatedNotes.map((n, idx) => {
                  const patMatch = patients?.find((p) => p.uhid.toLowerCase().trim() === (n.patientUhid || '').toLowerCase().trim());
                  const isEmergencyNote = Boolean(
                    patMatch?.isEmergency ||
                    (patMatch?.status || '').toLowerCase() === 'emergency' ||
                    (patMatch?.category || '').toLowerCase() === 'emergency' ||
                    n.patientCondition === 'Critical'
                  );
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4">
                      <span className={`font-bold ${isEmergencyNote ? 'text-rose-700' : 'text-blue-600'}`}>{n.patientCondition}</span>
                    </td>
      ...
      {selectedPatient?.uhid && (
        <div className="pt-2">
          <PatientHistoryViewer
            patientUhid={selectedPatient.uhid}
            patientName={`${selectedPatient.firstName} ${selectedPatient.lastName}`}
            title="Complete Patient Medical History & Clinical Logs"
            subtitle="Review patient past OPD consultations, diagnoses, prescriptions, and historical vitals"
