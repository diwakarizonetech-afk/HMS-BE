...
export const PatientBookingPage: React.FC = () => {
  ...
  const matchingPatients = useMemo(() => {
    if (!searchMobile.trim() || searchMobile.trim().length < 3) return [];
      ...
      <main className="flex-grow max-w-[1400px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-8">
        ...
        {currentStep === 2 && (
          <div className="w-full space-y-5 animate-fadeIn">
            <div className="flex items-center justify-between pb-2 border-b border-slate-200">
              <div>
                <h2 className="text-xl font-bold text-slate-900">Select Department & Doctor</h2>
                <p className="text-xs text-slate-500">Choose specialty department and specialist doctor.</p>
              </div>
              <span className="text-xs font-semibold bg-blue-100 text-blue-700 px-3 py-1 rounded-full">
                Step 2 of 4
              </span>
                ...
                {departmentDoctors.map((doc) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-slate-100 text-xs">
                      ...
                      <span className="font-black text-emerald-700">₹{doc.consultationFee || 500}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {timeSlotCategories.map((cat) => (
                  <div key={cat.title} className="bg-white p-4 rounded-xl border border-slate-200 space-y-2">
                    <h4 className="text-xs font-bold text-slate-600 uppercase tracking-wider">{cat.title}</h4>
                      ...
                      {cat.slots.map((slot) => {
                        ...
                        return (
                          <button
                            ...
                            className={`p-2.5 rounded-lg border text-xs font-bold text-center transition-all cursor-pointer ${isSelected
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-1">
                  ...
                  <p className="font-black text-slate-900 text-base">₹{selectedDoctor?.consultationFee || 500}</p>
                  <p className="text-[10px] text-emerald-600 font-bold">Pay at Hospital OPD Counter</p>
                </div>
              </div>
        ...
        {currentStep === 5 && confirmedApt && (
          <div className="max-w-xl mx-auto space-y-5 animate-fadeIn">
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-xl space-y-6 text-center">
              ...
              <div className="space-y-1">
                <span className="text-[10px] font-bold uppercase tracking-wider bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full border border-emerald-200">
                  Appointment Confirmed
                </span>
                <h2 className="text-2xl font-black text-slate-900 pt-2">Booking Slip Issued</h2>
                <p className="text-xs text-slate-500">Your appointment has been successfully scheduled.</p>
              </div>
              ...
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 text-xs text-left space-y-3">
                ...
                <div className="flex justify-between border-b border-slate-200 pb-2">
                  ...
                  <span className="font-bold text-slate-800">{confirmedApt.patientUhid}</span>
                </div>
                <div className="flex justify-between border-b border-slate-200 pb-2">
                ...
                <div className="flex justify-between border-b border-slate-200 pb-2">
                  ...
                  <span className="font-bold text-slate-800">{confirmedApt.doctorName}</span>
                </div>
                <div className="flex justify-between border-b border-slate-200 pb-2">
                  <span className="text-slate-400">Department</span>
                  <span className="font-bold text-blue-600">{confirmedApt.department}</span>
                </div>
                <div className="flex justify-between border-b border-slate-200 pb-2">
                  <span className="text-slate-400">Date & Time</span>
                  <span className="font-black text-emerald-700">{confirmedApt.date} @ {confirmedApt.timeSlot}</span>
                </div>
              </div>
