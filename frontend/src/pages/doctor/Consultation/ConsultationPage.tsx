...
import {
  ...
  Check, Download, Clock, Calendar, Building2
} from 'lucide-react';
...
export const ConsultationPage: React.FC = () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data)) {
            const myNameClean = (user?.name || '').toLowerCase().replace(/^dr\.\s*/i, '').trim();
            const myId = user?.id || '';
            ...
            const filteredData = data.filter((a: any) => {
              ...
              if (a.doctorId && a.doctorId !== myId) return false;
              if (a.doctor_id && a.doctor_id !== myId) return false;
              if (aDocClean && myNameClean && !aDocClean.includes(myNameClean) && !myNameClean.includes(aDocClean)) return false;
              return false;
            });

            const mapped: DoctorAppointment[] = filteredData.map((a: any) => {
              ...
              return {
                ...
                priority: a.priority !== undefined ? Number(a.priority) : (a.is_emergency || a.isEmergency ? 1 : 0),
                branch: a.branch || a.hospital_branch || a.hospitalBranch || '',
              };
            });
  ...
  useEffect(() => {
    const fetchSavedConsultations = async () => {
      try {
        const rows = await fetchConsultationsApi(user?.id);
  ...
  const handleOpenReportModal = (testName: string) => {
    ...
    if (!matchingReport && selectedAppointment && labTests.length > 0) {
      createPatientOrderFromOPD(
        ...
        selectedAppointment.branch || user?.branch
      );
    }
  ...
  const sortedAppointments = [...filteredAppointments].sort((a, b) => {
    // 1. Completed consultations last
    const isACompleted = a.status === 'Completed';
    const isBCompleted = b.status === 'Completed';
    if (isACompleted !== isBCompleted) {
      return isACompleted ? 1 : -1;
    }
    // 2. Active emergency appointments FIRST PRIORITY
    const isAEmergency = Boolean(a.isEmergency || (a.priority !== undefined && a.priority > 0));
    const isBEmergency = Boolean(b.isEmergency || (b.priority !== undefined && b.priority > 0));
  ...
  const handleSaveConsultation = () => {
    ...
    const doctorBranch = selectedAppointment.branch || user?.branch || 'Main Branch';
    ...
    (async () => {
      ...
      if (medicines.length > 0) {
        try {
          ...
          // Dispatch notification to same branch Pharmacy for dispensing
          sendNotification({
            title: '💊 New Prescription for Dispensing',
            message: `Dr. ${doctorDisplayName} sent a prescription for ${selectedAppointment.patientName} (${selectedAppointment.patientUhid}) to ${doctorBranch} pharmacy.`,
            type: 'info',
            module: 'Pharmacy',
            eventType: 'new_prescription',
            senderName: doctorDisplayName,
            recipientRole: 'pharmacist',
            relatedRecordId: selectedAppointment.patientUhid,
            priority: selectedAppointment.isEmergency ? 'urgent' : 'medium',
          });
        } catch (rxErr) {
                  ...
                  sortedAppointments.map((apt) => (
                    <tr
                      ...
                      className={`transition-colors ${
                        apt.isEmergency || (apt.priority !== undefined && apt.priority > 0)
  ...
  return (
    <div className="space-y-6 w-full max-w-7xl mx-auto">
      ...
      <div className={`bg-white p-6 rounded-2xl border shadow-2xs transition-all ${
        selectedAppointment?.isEmergency ? 'border-rose-400 bg-rose-50/20 ring-2 ring-rose-500/20' : 'border-slate-200'
      }`}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className={`w-14 h-14 rounded-2xl text-white font-bold flex items-center justify-center text-lg shrink-0 shadow-md ${
              selectedAppointment?.isEmergency
                ? 'bg-linear-to-tr from-rose-600 to-red-600 shadow-rose-600/30 animate-pulse'
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="inline-flex items-center gap-1 text-[11px] font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 px-2.5 py-0.5 rounded-full">
                  ...
                  {selectedAppointment?.branch || 'Main Campus (BKC)'}
                </span>
                {selectedAppointment?.isEmergency && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black bg-rose-600 text-white shadow-sm animate-pulse tracking-wide uppercase">
                    EMERGENCY PATIENT
                  </span>
                )}
              </div>
      ...
      {!isEditing ? (
        ...
        <div className="space-y-6">
          ...
          {/* Complete Patient Medical History & Previous Records (End of Page) */}
          {selectedAppointment?.patientUhid && (
            <div className="pt-2">
              <PatientHistoryViewer
                patientUhid={selectedAppointment.patientUhid}
                patientName={selectedAppointment.patientName}
                title="Historical Medical Records & Past Visit Details"
                subtitle="Complete cross-branch history: past doctor consultations, clinical diagnoses, past prescriptions, vitals trends, and lab investigation reports"
