...
import {
  ...
  AlertCircle, FileText, Upload, X, Filter, Inbox,
} from 'lucide-react';
...
const FilterDropdown: React.FC<{ label: string; value: string; options: string[]; onChange: (val: string) => void }> = ({ label, value, options, onChange }) => (
...
export const LeavePage: React.FC = () => {
  ...
  // Load this doctor's own leave requests from the shared /leaves backend
  // (the same table Super Admin's Leave Approval screen reads/writes).
  useEffect(() => {
    const loadLeaves = async () => {
      try {
        ...
        const mine = (Array.isArray(rows) ? rows : []).filter((r: any) => {
          if (myId && (r.employee_id === myId || r.employeeId === myId)) return true;
          const empNameClean = (r.employee_name || r.employeeName || '').toLowerCase().replace(/^dr\.\s*/i, '').trim();
          if (empNameClean && myNameClean && (empNameClean === myNameClean || empNameClean.includes(myNameClean) || myNameClean.includes(empNameClean))) return true;
          return false;
        });
        const mapped: LeaveRequest[] = mine.map((r: any) => ({
          id: r.id,
          doctorId: r.employee_id,
          doctorName: r.employee_name,
        ...
        console.warn('LeavePage: could not fetch leave requests:', e);
      } finally {
        setLoading(false);
      }
    };
    ...
    (async () => {
      ...
      try {
        const created = await createLeaveApi({
          ...
          employeeName: user?.name || 'Doctor',
          role: 'Doctor',
          department: (user as any)?.department || 'Medical',
          leaveType,
          fromDate: startDate,
          toDate: endDate,
          totalDays,
        ...
        const newRequest: LeaveRequest = {
          id: created.id,
          doctorId: created.employee_id,
          doctorName: created.employee_name,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Apply for leave and track approval status.</p>
        </div>
        <button
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-2xs space-y-4">
        <div className="flex items-center justify-between pb-2 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500">Your past and pending leave requests</p>
          </div>
          <FilterDropdown
            label="Status"
            value={filterStatus}
