...
export const MedicalHistoryPage: React.FC = () => {
  ...
  const { patients: hmsPatients, branches } = useHMS();
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            const mappedRecords: IPDPatientRecord[] = data.map((adm: any) => ({
              ...
              uhid: adm.patient_uhid || adm.patientUhid || undefined,
              branch: adm.branch || undefined,
            }));
  ...
  const filteredPatients = patients.filter((p) => {
    ...
    const matchesSearch =
      !qLower ||
      p.patientName.toLowerCase().includes(qLower) ||
      p.patientId.toLowerCase().includes(qLower) ||
      p.ipNumber.toLowerCase().includes(qLower) ||
      p.ward.toLowerCase().includes(qLower) ||
      p.admissionDiagnosis.toLowerCase().includes(qLower) ||
      (p.branch && p.branch.toLowerCase().includes(qLower));
    ...
    const matchesBranch =
      branchFilter === 'All' ||
      !p.branch ||
      p.branch.toLowerCase().trim() === branchFilter.toLowerCase().trim();
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {filteredPatients.length === 0 ? (
                    ...
                    filteredPatients.map((p, idx) => {
                      const isEmergency = Boolean((p as any).isEmergency || p.ward?.toLowerCase().includes('icu') || p.admissionDiagnosis?.toLowerCase().includes('emergency'));
                      return (
                      <tr
                        key={`${p.patientId}-${p.ipNumber}-${idx}`}
                        className={`transition-colors ${
                          isEmergency ? 'bg-rose-50/40 border-l-4 border-l-rose-500 hover:bg-rose-50/70' : 'hover:bg-blue-50/30'
                        }`}
                      >
                        {/* IP Number & Bed */}
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div className={`w-9 h-9 rounded-xl text-white font-bold flex items-center justify-center text-xs shrink-0 shadow-xs ${
                              isEmergency ? 'bg-gradient-to-tr from-rose-600 to-red-600' : 'bg-gradient-to-tr from-blue-600 to-indigo-600'
                            }`}>
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="inline-flex items-center gap-1 text-[11px] font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 px-2.5 py-0.5 rounded-md">
                ...
                {patient.branch || 'Main Campus (BKC)'}
              </span>
            </div>
        ...
        <div className="lg:col-span-9 bg-white rounded-2xl border border-slate-200 shadow-2xs p-6">
          ...
          {activeSection === 'discharge_summary' && (
            <div className="space-y-6">
              ...
              {/* Complete Cross-Branch Patient History & Previous Records */}
              {(patient?.uhid || patient?.patientId) && (
                <div className="pt-2">
                  <PatientHistoryViewer
                    patientUhid={patient.uhid || patient.patientId}
                    patientName={patient.patientName}
                    title="Cross-Branch Historical Medical Records"
                    subtitle="Prior doctor consultations, clinical diagnoses, past prescriptions, vitals trends, and lab reports across all visits"
