...
export const StaffShiftRosterPage: React.FC<StaffShiftRosterPageProps> = ({ portalRole }) => {
  ...
  // Filter shift rotation for the current logged-in employee or portal role
  const myShiftCandidate: ShiftRotation | undefined = shiftRotations.find((s) => {
    if (user) {
      const uName = (user.name || '').toLowerCase();
      const uId = (user.id || '').toLowerCase();
      const uEmpId = ((user as any).employeeId || (user as any).userId || '').toLowerCase();
      const sEmpId = (s.employeeId || '').toLowerCase();
      const sName = (s.employeeName || '').toLowerCase();

      if (uId && sEmpId === uId) return true;
      if (uEmpId && sEmpId === uEmpId) return true;
      if (uName && (sName.includes(uName) || uName.includes(sName))) return true;
    }
    if (currentRole === 'nurse') {
      return s.department.toLowerCase().includes('icu') || s.department.toLowerCase().includes('nurs') || s.employeeName.toLowerCase().includes('nurse');
    }
    if (currentRole === 'store') {
      return s.department.toLowerCase().includes('store') || s.department.toLowerCase().includes('inventory');
    }
    if (currentRole === 'doctor') {
      return s.department.toLowerCase().includes('opd') || s.department.toLowerCase().includes('med') || s.employeeName.toLowerCase().includes('dr');
    }
    if (currentRole === 'lab') {
      return s.department.toLowerCase().includes('lab') || s.department.toLowerCase().includes('pathology');
    }
    if (currentRole === 'pharmacy') {
      return s.department.toLowerCase().includes('pharmacy');
    }
    // Reception
    return s.department.toLowerCase().includes('front') || s.department.toLowerCase().includes('reception');
  });
  ...
  const myShift: ShiftRotation | undefined = myShiftCandidate || displayShifts[0];
