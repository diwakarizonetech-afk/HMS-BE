...
export const StaffShiftRosterPage: React.FC<StaffShiftRosterPageProps> = ({ portalRole }) => {
  ...
  // Filter shift rotation for the current logged-in employee specifically
  ...
  const myShift: ShiftRotation | undefined = myShiftCandidate;

  // Filter department shift roster (or all shifts if no department-specific roster exists)
  const currentDept = myShift?.department || (user as any)?.department;
  const deptShifts = currentDept
    ? shiftRotations.filter(
        (s) => s.department && s.department.toLowerCase() === currentDept.toLowerCase()
      )
    : [];
  const displayShifts = deptShifts.length > 0 ? deptShifts : shiftRotations;
              ...
              {displayShifts.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-slate-400">
                    No active shift schedules found.
                  </td>
                </tr>
              ) : (
                displayShifts.map((s) => (
