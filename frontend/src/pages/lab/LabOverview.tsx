...
export const LabOverview: React.FC = () => {
  ...
  const branchSamples = React.useMemo(() => sampleCollections.filter((s) => matchBranch(s.branch)), [sampleCollections, selectedBranch]);
  const branchProcessing = React.useMemo(() => sampleProcessingList.filter((p) => matchBranch(p.branch)), [sampleProcessingList, selectedBranch]);
  const branchResults = React.useMemo(() => labResults.filter((r) => matchBranch(r.branch)), [labResults, selectedBranch]);
  const branchReports = React.useMemo(() => labReports.filter((rep) => matchBranch(rep.branch)), [labReports, selectedBranch]);

  // Metric Computations based on branch-filtered datasets
  const totalTestOrders = branchSamples.length;
  const samplesCollectedToday = branchSamples.filter((s) => s.status === 'Collected').length;
  const samplesInProcessing = branchProcessing.filter((p) => p.status === 'In Processing').length;
  const completedTestsToday = branchResults.filter((r) => r.status === 'Completed' || r.status === 'Verified').length;
  const pendingReports = branchReports.filter((r) => r.status === 'Generated' || r.status === 'Draft').length;
  const criticalResultsCount = branchResults.filter((r) => r.flag === 'Critical' || r.status === 'Critical').length;
  const pendingDoctorReviews = branchReports.filter((r) => r.doctorReviewStatus === 'Pending Review').length;

  const totalCalculatedRevenue = branchResults
  ...
  const statusDistributionData = [
    { name: 'Pending', value: branchSamples.filter((s) => s.status === 'Pending').length, color: '#eab308' },
    { name: 'In Processing', value: samplesInProcessing, color: '#a855f7' },
    { name: 'Completed', value: branchResults.filter((r) => r.status === 'Completed').length, color: '#10b981' },
    { name: 'Verified', value: branchResults.filter((r) => r.status === 'Verified').length, color: '#3b82f6' },
  ];

  const daysOfWeek = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
  const weeklyPerformanceData = daysOfWeek.map((day) => ({
    day,
    collected: branchSamples.filter((s) => s.collectionDate && new Date(s.collectionDate).toLocaleDateString('en-US', { weekday: 'short' }) === day).length,
    completed: branchResults.filter((r) => r.entryDate && new Date(r.entryDate).toLocaleDateString('en-US', { weekday: 'short' }) === day).length,
  }));

  // Task Counts
  const pendingCollectionCount = branchSamples.filter((s) => s.status === 'Pending').length;
  const pendingProcessingCount = branchProcessing.filter((p) => p.status === 'Pending').length;
  const pendingResultEntryCount = branchProcessing.filter((p) => p.status === 'Completed').length;
  const pendingReportGenCount = branchResults.filter((r) => r.status === 'Completed' && r.verifiedBy !== 'Pending').length;
