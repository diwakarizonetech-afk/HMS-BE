...
import {
  ...
  AlertCircle, FileText, Upload, X, Filter, Inbox, Microscope,
} from 'lucide-react';
...
// Leave entitlements are organization policy constants (days per year)
...
// ─── Main Component ───────────────────────────────────────────
export const LabLeavePage: React.FC = () => {
  const { user } = useAuth();
  const [leaveRequests, setLeaveRequests] = useState<LeaveRequest[]>([]);

  // Compute statistics live from actual DB leave records
  const statistics = useMemo((): LeaveStatistics => {
    const countDays = (type: string) =>
      leaveRequests
        .filter((r) => r.leaveType === type && (r.status === 'Approved' || r.status === 'Pending'))
        .reduce((sum, r) => sum + (r.totalDays || 0), 0);
    return {
      ...LEAVE_ENTITLEMENTS,
      usedCasual: countDays('Casual Leave'),
      usedMedical: countDays('Medical Leave'),
      usedEmergency: countDays('Emergency Leave'),
      usedAnnual: countDays('Annual Leave'),
    };
  }, [leaveRequests]);
      ...
      try {
        ...
        const mine = (Array.isArray(rows) ? rows : []).filter(
          (r: any) =>
            ...
            (r.role && r.role.toLowerCase().includes('lab')) ||
        ...
        console.warn('LabLeavePage: could not fetch leave requests:', e);
      } finally {
        setLoading(false);
      }
    };
    ...
    (async () => {
      ...
      try {
        const created = await createLeaveApi({
          ...
          employeeName: user?.name || 'Lab Technician',
          role: 'Lab Technician',
          department: (user as any)?.department || 'Laboratory',
          leaveType,
          fromDate: startDate,
          toDate: endDate,
          totalDays,
        ...
        const newRequest: LeaveRequest = {
          ...
          role: created.role || 'Lab Technician',
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Lab Technician Leave Management</h1>
            <p className="text-xs text-slate-500 mt-0.5">
              Apply for lab staff leave and track your HOD approval status.
            </p>
          </div>
        </div>
        <button
          onClick={() => setShowForm(!showForm)}
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-cyan-600 hover:bg-cyan-700 shadow-md cursor-pointer transition-colors"
        >
      ...
      {/* ─── Apply Leave Form ──────────────────────────── */}
      {showForm && (
        <div className="bg-white p-6 rounded-2xl border border-cyan-200 shadow-2xs space-y-5">
          <div className="flex items-center gap-2 pb-3 border-b border-slate-100">
            <div className="w-8 h-8 rounded-lg bg-cyan-100 text-cyan-600 flex items-center justify-center">
              <FileText className="w-4 h-4" />
            </div>
            <h3 className="text-sm font-bold text-slate-900">Apply for Lab Technician Leave</h3>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-xs">
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-semibold outline-none focus:ring-2 focus:ring-cyan-500/20 cursor-pointer"
              >
                {LEAVE_TYPES.map((t) => (
            ...
            <div className="flex items-end gap-3">
              <div className="flex-1">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-semibold outline-none focus:ring-2 focus:ring-cyan-500/20"
                />
              </div>
              <div className="flex-1">
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                placeholder="Please provide reason for your lab leave request..."
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs outline-none focus:ring-2 focus:ring-cyan-500/20"
              />
            </div>
            ...
            <div className="flex items-end">
              {totalDays > 0 && (
                <div className="p-3 rounded-xl bg-cyan-50 border border-cyan-200 flex items-center gap-2">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-cyan-600 hover:bg-cyan-700 shadow-md cursor-pointer disabled:opacity-50 transition-colors"
            >
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-2xs space-y-4">
        <div className="flex items-center justify-between pb-2 border-b border-slate-100">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Lab Leave History</h3>
            <p className="text-xs text-slate-500">Your past and pending lab leave applications</p>
          </div>
          <FilterDropdown
            label="Status"
            value={filterStatus}
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {filteredRequests.map((leave) => (
                  <tr key={leave.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="p-3">
                      <span className="text-[10px] font-bold text-cyan-600 bg-cyan-50 px-2 py-0.5 rounded-full whitespace-nowrap">
                        {leave.leaveType}
                      </span>
                    </td>
