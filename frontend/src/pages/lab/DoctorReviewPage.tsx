...
export const DoctorReviewPage: React.FC = () => {
  const { labReports, labResults, doctorReviewReport, selectedBranch, matchBranch } = useLab();
  ...
  // Filtered List with Priority Sorting
  const filteredReports = useMemo(() => {
    const matched = branchReports.filter((rep) => {
      ...
      if (selectedDateFilter.trim() !== '') {
        matchesDate =
          ...
          Boolean(rep.doctorReviewDate && rep.doctorReviewDate.includes(selectedDateFilter));
      }
    ...
    return matched.sort((a, b) => {
      const isAApproved = a.doctorReviewStatus === 'Approved' || a.doctorReviewStatus === 'Rejected';
      const isBApproved = b.doctorReviewStatus === 'Approved' || b.doctorReviewStatus === 'Rejected';
      if (isAApproved !== isBApproved) {
        return isAApproved ? 1 : -1; // completed/approved data last
      }
  ...
  }, [branchReports, searchQuery, selectedReviewStatus, selectedDateFilter, patients]);
  ...
  const pendingReviewCount = branchReports.filter((r) => r.doctorReviewStatus === 'Pending Review').length;
  const reviewedCount = branchReports.filter((r) => r.doctorReviewStatus !== 'Pending Review').length;
  const approvedCount = branchReports.filter((r) => r.doctorReviewStatus === 'Approved').length;
  const rejectedCount = branchReports.filter((r) => r.doctorReviewStatus === 'Rejected' || r.doctorReviewStatus === 'Re-Test Requested').length;
              ...
              {paginatedReports.length > 0 ? (
                paginatedReports.map((rep) => {
                  ...
                  return (
                    ...
                    <td className="py-3 px-4 max-w-50" title={rep.tests.join(', ')}>
                      <div className="flex flex-wrap gap-1">
                        {rep.tests.map((t, i) => (
                          <span
                            key={i}
                            className="bg-slate-100 text-slate-700 text-[10px] px-2 py-0.5 rounded-full font-medium truncate max-w-35"
                          >
                            {t}
                          </span>
                        ))}
                    ...
                    <td className="py-3 px-4 max-w-55" title={rep.doctorComments}>
                      {rep.doctorComments ? (
