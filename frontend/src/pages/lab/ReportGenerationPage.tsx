...
export const ReportGenerationPage: React.FC = () => {
  const { labReports, testMasterList, labResults, updateReportStatus, updateLabReport, doctorReviewReport, getTechniciansByBranch, selectedBranch, matchBranch } = useLab();
  ...
  // Branch-Filtered Dataset
  const branchReports = useMemo(() => {
    return labReports.filter((rep) => matchBranch(rep.branch));
  }, [labReports, selectedBranch]);

  // Filtered List with Date Filter and Priority Ordering
  const filteredReports = useMemo(() => {
    const matched = branchReports.filter((rep) => {
      const matchesSearch =
    ...
    return matched.sort((a, b) => {
      const isACompleted = a.status === 'Printed' || a.status === 'Emailed' || a.status === 'Approved';
      const isBCompleted = b.status === 'Printed' || b.status === 'Emailed' || b.status === 'Approved';
      ...
      const patB = patients?.find((p) => b.patientUhid && p.uhid.toLowerCase().trim() === b.patientUhid.toLowerCase().trim());
      const isAEmergency = Boolean(a.isEmergency || patA?.isEmergency || (patA?.status || '').toLowerCase() === 'emergency');
      const isBEmergency = Boolean(b.isEmergency || patB?.isEmergency || (patB?.status || '').toLowerCase() === 'emergency');
      if (isAEmergency !== isBEmergency) {
        return isAEmergency ? -1 : 1; // emergency first
      }
      return 0;
    });
  }, [branchReports, searchQuery, selectedStatus, selectedDateFilter, patients]);
  ...
  // Stat Counters based on branch-filtered reports
  const generatedCount = branchReports.length;
  const pendingCount = branchReports.filter((r) => r.status === 'Draft' || r.status === 'Generated').length;
  const printedCount = branchReports.filter((r) => r.status === 'Printed').length;
  const emailedCount = branchReports.filter((r) => r.status === 'Emailed').length;
  ...
  const handleOpenEditModal = (rep: LabReportItem) => {
    ...
    const branchTechs = getTechniciansByBranch(rep.branch);
    const initialTech = rep.generatedBy || (rep.testResults && rep.testResults[0]?.technician) || branchTechs[0]?.name || 'Lab Technician';
  ...
  const handleSaveReportEdit = async (e: React.FormEvent) => {
    ...
    const updatedTestResults = editForm.testResults.map((tr, idx) => {
      ...
      return {
        ...
        flag: tr.flag,
        technician: editForm.technician || original?.technician || activeReport.generatedBy || '',
    ...
    const updated: LabReportItem = {
      ...activeReport,
      generatedBy: editForm.technician || activeReport.generatedBy,
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {paginatedReports.length > 0 ? (
                paginatedReports.map((rep) => {
                  ...
                  const isEmergencyReport = Boolean(
                    rep.isEmergency ||
                    ...
                    <tr
                      ...
                      className={`transition-colors ${
                        isEmergencyReport
                          ...
                          : rep.status === 'Printed' || rep.status === 'Emailed' || rep.status === 'Approved'
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 space-y-6 shadow-2xl border border-slate-100 animate-scale-up my-8">
            ...
            <form onSubmit={handleSaveReportEdit} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80">
                <div>
                ...
                <div>
                  ...
                  <span className="font-bold text-purple-700">{activeReport.branch || 'Main Branch'}</span>
                </div>
              </div>

              {/* Lab Technician Selector */}
              <div>
                <label className="block font-bold text-slate-700 mb-1">
                  Responsible Lab Technician ({activeReport.branch || 'Main Branch'})
                </label>
                <select
                  value={editForm.technician}
                  onChange={(e) => setEditForm({ ...editForm, technician: e.target.value })}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 font-bold text-slate-800 text-xs focus:bg-white"
                >
                  {getTechniciansByBranch(activeReport.branch).map((t) => (
                    <option key={t.id || t.name} value={t.name}>
                      👨‍🔬 {t.name} {t.department ? `(${t.department})` : ''} - {t.branch || 'Main Branch'}
                    </option>
                  ))}
                </select>
              </div>
