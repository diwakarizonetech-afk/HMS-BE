...
export const ReportGenerationPage: React.FC = () => {
  const { labReports, updateReportStatus } = useLab();
    ...
    setEditForm({
      ...
      testResults:
        rep.testResults && rep.testResults.length > 0
          ? rep.testResults.map((tr) => ({ ...tr }))
          : [
              {
                id: 'tr-1',
                testName: rep.tests[0] || 'Complete Blood Count (CBC)',
                resultValue: '14.2',
                unit: 'g/dL',
  ...
  const handleSaveReportEdit = (e: React.FormEvent) => {
    ...
    const updated: LabReportItem = {
      ...
      testResults: editForm.testResults.map((tr, idx) => {
        // Merge the editable fields back onto the original full LabResultItem
        // (patientName/patientUhid/testCode/sampleId/technician/verifiedBy),
        // falling back to sensible defaults for a newly-added row.
    ...
    // Sync in global lab reports list
    const foundIndex = labReports.findIndex((r) => r.id === activeReport.id);
    if (foundIndex !== -1) {
      labReports[foundIndex] = updated;
    }

    addToast('success', 'Report Updated', `Lab report ${activeReport.reportNumber} saved successfully.`);
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-slate-100">
                  {activeReport.testResults && activeReport.testResults.length > 0 ? (
                    ...
                    <tr>
                      <td className="py-2.5 px-3 font-bold text-slate-900">Complete Blood Count (CBC)</td>
                      <td className="py-2.5 px-3 font-extrabold text-sm text-slate-900">14.2</td>
                      <td className="py-2.5 px-3 text-slate-600">g/dL</td>
                      <td className="py-2.5 px-3 text-slate-600">13.5 - 17.5 g/dL</td>
                      <td className="py-2.5 px-3">
            ...
            <div className="pt-4 border-t border-slate-200 grid grid-cols-2 gap-6 text-xs">
              ...
              <div className="text-right space-y-1">
                <div className="inline-block p-2 border-b-2 border-slate-800 font-serif italic text-sm text-slate-800">
                  Dr. Suresh Mehta, MD (Path)
                </div>
                <p className="text-[10px] font-bold text-slate-600">Chief Consultant Pathologist</p>
