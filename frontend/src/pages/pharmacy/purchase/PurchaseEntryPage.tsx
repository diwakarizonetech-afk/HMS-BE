...
export const PurchaseEntryPage: React.FC = () => {
  ...
  const { addPurchase } = usePharmacy();

  // Form Top Metadata
  const [purchaseNo, setPurchaseNo] = useState(`PO-${new Date().getFullYear()}-${Math.floor(100 + Math.random() * 900)}`);
  ...
  const [inputMrp, setInputMrp] = useState(0);
  ...
  const handleSavePurchase = async (status: 'Completed' | 'Draft') => {
    if (items.length === 0) {
    ...
    try {
      ...
      addToast('error', 'Save Failed', 'Failed to persist purchase entry to server.');
    }
  };
