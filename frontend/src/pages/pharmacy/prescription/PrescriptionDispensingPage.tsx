...
export const PrescriptionDispensingPage: React.FC = () => {
  ...
  const { prescriptions: initialPrescriptions } = usePharmacy();
  ...
  const filteredPrescriptions = prescriptions.filter((rx) => {
    const matchesSearch =
      ...
      rx.patientUhid.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const calculateRxTotal = (rx: PrescriptionOrder) => {
    return rx.items.reduce((acc, item) => acc + item.price, 0);
  };
  ...
  const handleSubmitDispensing = () => {
    if (!selectedRx) return;
    setPrescriptions((prev) =>
      prev.map((item) => (item.id === selectedRx.id ? selectedRx : item))
    );
                  ...
                  <tr key={rx.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-4">
                      ...
                      >
                        ...
                        {pStatus === 'IPD Credit / Post Bill' && `IPD Credit`}
                      </span>
                    </td>
