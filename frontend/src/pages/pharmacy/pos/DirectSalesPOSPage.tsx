...
export const DirectSalesPOSPage: React.FC = () => {
  ...
  const filteredMedicines = medicines.filter((m) => {
    ...
    const matchesBarcode = !barcode || m.code.toLowerCase().includes(barcode);
  ...
  const filteredInvoices = recentInvoices.filter((inv) => {
    ...
    const matchesSearch =
      ...
      inv.items.some((i) => i.medicineName.toLowerCase().includes(query));
  ...
  const handleCheckout = () => {
    ...
    const newInvoice: POSInvoice = {
      id: `inv-${Date.now()}`,
      invoiceNumber: `POS-2026-00${Math.floor(415 + Math.random() * 50)}`,
    ...
    addToast(
      'success',
      'POS Sale Completed! 🎉',
      `Invoice ${newInvoice.invoiceNumber} recorded. Total Paid: ₹${grandTotal.toFixed(2)} (${paymentMethod})`
    );
  };
