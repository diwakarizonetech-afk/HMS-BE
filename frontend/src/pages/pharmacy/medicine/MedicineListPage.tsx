...
export const MedicineListPage: React.FC = () => {
  const { medicines: contextMedicines, categories } = usePharmacy();
  ...
  const handleDeleteMedicine = (id: string) => {
    if (confirm('Are you sure you want to delete this medicine from Master?')) {
      ...
      fetch(`${import.meta.env.VITE_API_URL}/api/v1/pharmacy/medicines/${id}`, {
        method: 'DELETE',
  ...
  const handleSaveMedicine = (e: React.FormEvent) => {
    ...
    if (isEditMode && selectedMedicine) {
      ...
      setMedicines((prev) =>
        prev.map((item) =>
          item.id === selectedMedicine.id ? { ...item, ...updatedItem } : item
        )
      );
      const token = localStorage.getItem('hms_token');
      fetch(`${import.meta.env.VITE_API_URL}/api/v1/pharmacy/medicines/${selectedMedicine.id}`, {
        method: 'PUT',
      ...
      const token = localStorage.getItem('hms_token');
      fetch(`${import.meta.env.VITE_API_URL}/api/v1/pharmacy/medicines`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        ...
        if(r.ok) {
           ...
           setMedicines(prev => prev.map(m => m.id === newMed.id ? { ...m, id: saved.id } : m));
        }
      }).catch(console.error);
    }
