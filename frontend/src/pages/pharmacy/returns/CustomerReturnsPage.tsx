...
export const CustomerReturnsPage: React.FC = () => {
  const { customerReturns: contextReturns, addCustomerReturn } = usePharmacy();
  ...
  const handleSaveReturn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fallbackRet: CustomerReturn = {
        id: `cret-${Date.now()}`,
        ...payload,
      };
      setReturns((prev) => [fallbackRet, ...prev]);
    }
