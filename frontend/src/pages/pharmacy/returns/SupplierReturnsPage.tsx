...
export const SupplierReturnsPage: React.FC = () => {
  const { supplierReturns: contextReturns, addSupplierReturn } = usePharmacy();
  const [returns, setReturns] = useState<SupplierReturn[]>(contextReturns);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      medicineName: medName || 'Formulation Item',
      batchNumber: batchNo,
      quantity: Number(qty),
      reason,
      creditNoteNo: creditNoteNo || `CN-${Date.now()}`,
      amount: Number(amount),
      status: 'Pending Credit' as const,
      date: new Date().toISOString().split('T')[0],
    };
    try {
      await addSupplierReturn(payload);
    } catch (err) {
      console.error('Failed to save supplier return:', err);
      const fallbackSret: SupplierReturn = {
        id: `sret-${Date.now()}`,
        ...payload,
      };
      setReturns((prev) => [fallbackSret, ...prev]);
    }
    setModalOpen(false);
  };
